...
describes what users get. Pick a format with `-o`, or let v2ex choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
v2ex <command> -o table   # aligned columns for reading
v2ex <command> -o jsonl   # one JSON object per line, for piping
v2ex <command> -o json    # a single JSON array
v2ex <command> -o csv     # spreadsheet friendly
v2ex <command> -o tsv     # tab-separated
v2ex <command> -o url     # just the URL column
v2ex <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
v2ex <command> --fields id,title,url
```
...
```bash
v2ex <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
v2ex <command>            # a table, because this is a terminal
v2ex <command> | wc -l    # JSONL, because this is a pipe
```
